...
const RedeemRewardsTab = ({ 
  ...
  const filterOptions = [
    ...
    { key: REWARD_FILTERS.UNLOCKED, label: 'Unlocked', count: rewards.filter(r => userPoints >= r.cost).length },
    { key: REWARD_FILTERS.LOCKED, label: 'Locked', count: rewards.filter(r => userPoints < r.cost).length },
    { key: REWARD_FILTERS.COMING_SOON, label: 'Coming Soon', count: 0 },
  ];

  const getFilteredRewards = () => {
    if (rewardFilter === REWARD_FILTERS.ALL) return rewards;
    if (rewardFilter === REWARD_FILTERS.UNLOCKED) return rewards.filter(r => userPoints >= r.cost);
    if (rewardFilter === REWARD_FILTERS.LOCKED) return rewards.filter(r => userPoints < r.cost);
        ...
        {filterOptions.map(filter => (
          <button
            ...
            className={`pb-3 px-1 font-medium transition-colors relative ${
              rewardFilter === filter.key ? 'text-purple-600' : 'text-gray-500 hover:text-gray-700'
            }`}
          >
            {filter.label} <span className="text-gray-400 ml-1">{filter.count}</span>
